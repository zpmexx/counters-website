...
const ShopListElement = ({ text, bgColor, onClick, roundedClass }) => {
  const baseClasses = `text-xl font-light text-sm p-1 w-full text-center bg-${bgColor} ${roundedClass} transition-all select-none ${(bgColor === "faded" || bgColor === "secondary") ? 'text-white' : ''}`;
